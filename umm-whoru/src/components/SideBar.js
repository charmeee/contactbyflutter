...
function SideBarToggle(props){
    return(
        <a id="SideBarToggle" href="/open side bar" onClick={(event)=>{
...
function ProfileMenu(props){
    return(
        <div id="ProfileMenu">
            ...
            <p id="ProfileMenuIcon"><FontAwesomeIcon icon={faPenToSquare}/></p>
        </div>
    )
}
...
function Menu(props){
    return(
        <a class="Menu" href={props.link}>
            <p class="MenuIcon"><FontAwesomeIcon icon={props.icon}/></p>
...
function Menus(props){
    ...
    for(let i=0; i<menuList.length; i++){
        html.push(
            <Menu icon={menuList[i].icon} text={menuList[i].text} link={menuList[i].link}></Menu>
        )
...
function IntroMenu(){
    return(
        <div id="IntroMenu">
            <p id="IntroMenuText">
                소프트웨어 마에스트로 13기 미니 프로젝트<br/>
                팀 12 - 권순현, 김주원, 이동언, 전민지, 황재원
            </p>
        </div>
    )
}
...
function OpenedSideBar(props){
    const user = {image: "", name: "이동언", code: 473214}
    ...
    return(
        <div id="OpenedSideBar">
            <SideBarToggle onChange_sidebarState={()=>{props.onChange_sidebarState()}}></SideBarToggle>
            <DivisionLine></DivisionLine>
            <ProfileMenu image={user.code} name={user.name} code={user.code}></ProfileMenu>
            <DivisionLine></DivisionLine>
...
function Sidebar(){
    ...
    if(sidebarState === 'off'){
        return(
            <ClosedSideBar onChange_sidebarState={()=>{set_sidebarState('on')}}></ClosedSideBar>
        )
    }
    else{
        return(
            <OpenedSideBar onChange_sidebarState={()=>{set_sidebarState('off')}}></OpenedSideBar>
        )
    }
